...
def test_poll_default_feeds_returns_only_new_items():
    ...
    first = client.post("/poll", json={}).json()
    ...
    second = client.post("/poll", json={}).json()
...
def test_max_items_per_feed():
    ...
    r = client.post("/poll", json={"feeds": [FEED_A], "max_items_per_feed": 2}).json()
...
def test_bad_feed_does_not_fail_poll():
    ...
    r = client.post("/poll", json={"feeds": [FEED_A, FEED_B, "http://127.0.0.1/rss"]}).json()
    assert r["checked"] == 3
...
def test_no_feeds_is_422(monkeypatch, tmp_path):
    ...
    assert client.post("/poll", json={}).status_code == 422
